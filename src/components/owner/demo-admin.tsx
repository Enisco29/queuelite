...
export function DemoAdmin({ initialState }: { initialState: OwnerQueueState }) {
  ...
  return <main className="container-page max-w-4xl py-10">
    ...
    <div className="mt-7 flex items-end justify-between gap-4"><div><p className="eyebrow">Demo admin</p><h1 className="mt-2 text-4xl font-bold tracking-[-.04em]">{state.queue.name}</h1><div className="mt-3"><RealtimeStatus state={connection} onRefresh={() => void refreshNow().catch(() => setError("Could not refresh the queue."))} /></div></div><a className="button button-secondary" href="/q/demo" target="_blank">Open customer page</a></div>
